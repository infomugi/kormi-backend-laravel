            ...
            <div class="flex items-center gap-1.5 flex-wrap justify-center">
                ...
                @else
                    <button type="button" 
                        ...
                        class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-slate-600 bg-white border border-slate-200/80 rounded-xl hover:bg-slate-50 hover:border-slate-300 hover:text-indigo-600 active:scale-95 transition-all shadow-xs cursor-pointer">
                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" />
                        </svg>
                        <span class="hidden sm:inline">Sebelumnya</span>
                                ...
                                <span wire:key="paginator-{{ $paginator->getPageName() }}-page{{ $page }}">
                                    ...
                                    @else
                                        <button type="button" 
                                            ...
                                            x-on:click="{{ $scrollIntoViewJsSnippet }}" 
                                            class="inline-flex items-center justify-center min-w-[32px] h-8 px-2.5 text-xs font-semibold text-slate-600 bg-white border border-slate-200/80 rounded-xl hover:bg-slate-50 hover:border-slate-300 hover:text-indigo-600 active:scale-95 transition-all shadow-xs cursor-pointer">
                                            {{ $page }}
                                        </button>
                                    @endif
                                </span>
