...
class Sejarah extends Component
{
    ...
    {
        $dbMilestones = LinimasaSejarah::aktif()->urut()->get();

        if ($dbMilestones->isNotEmpty()) {
            $milestones = $dbMilestones->map(fn($item, $idx) => [
                'tahun' => $item->tahun ?? $item->tahun_periode ?? '',
                'judul' => $item->judul ?? $item->judul_peristiwa ?? '',
                'deskripsi' => $item->deskripsi ?? $item->deskripsi_peristiwa ?? '',
